<template>
    <nav class="text-center mt-16 md:mt-20 mb-10">
        <button type="button" @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave" @click="handleNavigate"  :to="`/challenge/${nextChallenge.id}`" 
            ...
            <div class="flex-grow md:w-1/2 order-first md:order-last flex justify-center items-center">
                <div class="w-full max-w-[400px] flex flex-row flex-wrap md:flex-col md:gap-10 justify-center font-heading text-center text-xl sm:text-3xl md:text-4xl">
                    <p class="uppercase md:text-left">Next -&nbsp;</p>
                    <p>{{ nextChallenge.name }}</p>
                </div>
            </div>
        </button>
    </nav>
